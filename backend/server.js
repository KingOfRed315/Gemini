...
// Cho phép tất cả các nguồn (giao diện Vercel) truy cập bảo mật
app.use(cors({ origin: '*' })); 
...
app.post('/api/chat', async (req, res) => {
    try {
        ...
        // Gửi request chuẩn cấu trúc dữ liệu mảng contents của Google
        ...
        // Kiểm tra lỗi từ phía Google trả về
        if (data.error) {
            console.error("Google Error:", data.error);
            return res.status(400).json({ reply: `Google báo lỗi: ${data.error.message}` });
        }

        // Bóc tách dữ liệu phản hồi văn bản từ JSON thành công
        if (data.candidates && data.candidates[0].content && data.candidates[0].content.parts) {
            const aiResponse = data.candidates[0].content.parts[0].text;
            return res.json({ reply: aiResponse });
        } else {
            console.error("Cấu trúc lạ:", data);
